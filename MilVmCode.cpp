...
bool Code::translateProc(Procedure& proc)
{
    ...
    if( !locals.isEmpty() )
    {
        proc.localsSize = locals.last()->off + locals.last()->getType()->getByteSize(pointerWidth);
    ...
    if( !params.isEmpty() )
        ...
        proc.argsSize = stackAligned(params.last()->off + params.last()->getType()->getByteSize(pointerWidth));
    if( proc.decl->getType() )
        proc.returnSize = stackAligned(proc.decl->getType()->getByteSize(pointerWidth));
...
bool Code::translateStat(Procedure &proc, Statement *& s)
{
    if( s == 0 )
        return true;
    switch(s->kind)
    ...
    case IL_stloc_3:
        ...
        break;
    case IL_stind:
        emitOp(proc, LL_stind_vt,deref(s->d->getType())->getByteSize(pointerWidth));
        break;
    case IL_stind_i1:
    ...
    case IL_stind_ip:
        ...
        break;
    case IL_stind_ipp:
        emitOp(proc, LL_stind_vt,2 * pointerWidth);
        break;
    case IL_stelem_ipp:
        emitOp(proc, LL_stelem_vt, 2 * pointerWidth);
        break;
    case IL_stelem:
        emitOp(proc, LL_stelem_vt, deref(s->d->getType())->getByteSize(pointerWidth));
        break;
    case IL_stelem_i1:
    ...
    case IL_stfld:
        ...
        case Type::Array:
            ...
            emitOp(proc, LL_vt_size, deref(s->d->getType())->getByteSize(pointerWidth));
            break;
        default:
            Q_ASSERT(false);
            break;
    ...
    case IL_while:
        ...
        break;
    case IL_pop:
        emitOp(proc, LL_pop, s->args->getType()->getByteSize(pointerWidth));
        break;
    case IL_strcpy:
        emitOp(proc, LL_strcpy, deref(s->args->lhs->getType()->getType())->len);
        break;
    case IL_ret:
        emitOp(proc, LL_ret, s->args ? s->args->getType()->getByteSize(pointerWidth) : 0 );
        break;
    case IL_free:
        emitOp(proc, LL_free );
        break;
...
bool Code::translateStatExpr(Procedure &proc, Statement *s)
{
    Expression* e = s->e;
    return translateExprSeq(proc, e);
}
...
bool Code::translateExpr(Procedure& proc, Expression* e)
{
    ...
    case IL_ldind_ip:
        ...
        break;
    case IL_ldind_ipp:
        emitOp(proc, LL_ldind_vt,2 * pointerWidth);
        break;
    case IL_ldind_r4:
    ...
    case IL_ldelem_u8:
        ...
        break;
    case IL_ldelem_ipp:
        emitOp(proc, LL_ldelem_vt,2 * pointerWidth);
        break;
    case IL_ldelem:
    ...
    case IL_callmi: {
        // Compute argsSize and returnSize from the MethRef's procedure type.
