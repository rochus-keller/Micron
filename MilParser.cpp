...
static inline bool FIRST_DeclarationSequence(int tt) {
	return tt == Tok_PROC || tt == Tok_CONST || tt == Tok_PROCEDURE || tt == Tok_VAR || tt == Tok_TYPE;
}
...
void Parser::ProcedureDeclaration(SynTree* st) {
	...
	if( la.d_type == Tok_Lpar ) {
		if( expect(Tok_Lpar, false, "ProcedureDeclaration") ) addTerminal(st);
		if( expect(Tok_ident, false, "ProcedureDeclaration") ) addTerminal(st);
		if( expect(Tok_Colon, false, "ProcedureDeclaration") ) addTerminal(st);
		if( expect(Tok_ident, false, "ProcedureDeclaration") ) addTerminal(st);
		if( expect(Tok_Rpar, false, "ProcedureDeclaration") ) addTerminal(st);
		identdef(st);
		if( FIRST_FormalParameters(la.d_type) ) {
			FormalParameters(st);
		}
...
void Parser::module(SynTree* st) {
	...
	while( FIRST_ImportList(la.d_type) || FIRST_DeclarationSequence(la.d_type) ) {
		if( FIRST_ImportList(la.d_type) ) {
			ImportList(st);
		} else if( FIRST_DeclarationSequence(la.d_type) || la.d_type == Tok_CONST || la.d_type == Tok_END || la.d_type == Tok_IMPORT || la.d_type == Tok_PROC || la.d_type == Tok_VAR || la.d_type == Tok_TYPE || la.d_type == Tok_PROCEDURE ) {
...
void Parser::import(SynTree* st) {
	...
	while( ( peek(1).d_type == Tok_ident && peek(2).d_type == Tok_Slash )  ) {
		...
		if( expect(Tok_Slash, false, "import") ) addTerminal(st);
	}
	if( expect(Tok_ident, false, "import") ) addTerminal(st);
...
void Parser::ExpInstr(SynTree* st) {
	...
	} else if( la.d_code == Tok_CALL || la.d_code == Tok_CALLI || la.d_code == Tok_CALLVIRT ) {
	...
	} else if( la.d_code == Tok_LDPROC || la.d_code == Tok_LDMETH ) {
		if( la.d_code == Tok_LDPROC ) {
...
void Parser::Statement(SynTree* st) {
	if( la.d_code == Tok_FREE ) {
		...
		if( expect(Tok_IFGOTO, true, "Statement") ) addTerminal(st);
		if( expect(Tok_ident, false, "Statement") ) addTerminal(st);
...
void Parser::ConstExpression2(SynTree* st) {
	...
	if( FIRST_qualident(la.d_type) ) {
		...
		if( expect(Tok_real, false, "ConstExpression2") ) addTerminal(st);
	} else if( la.d_type == Tok_string ) {
