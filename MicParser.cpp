...
static inline bool FIRST_FixedPart(int tt) {
	return tt == Tok_INLINE || tt == Tok_IN || tt == Tok_2Dot || tt == Tok_ident;
}
...
static inline bool FIRST_selector(int tt) {
	return tt == Tok_Hat || tt == Tok_Dot || tt == Tok_Lbrack || tt == Tok_Lpar;
}
...
static inline bool FIRST_InterfaceProc(int tt) {
	return tt == Tok_PROC || tt == Tok_PROCEDURE || tt == Tok_ident;
}
...
static inline bool FIRST_ForwardDecl(int tt) {
	return tt == Tok_PROC || tt == Tok_PROCEDURE;
}
...
static inline bool FIRST_DeclarationSequence(int tt) {
	return tt == Tok_TYPE || tt == Tok_PROC || tt == Tok_PROCEDURE || tt == Tok_CONST || tt == Tok_VAR;
}
...
static inline bool FIRST_ModuleAttributes(int tt) {
...
static inline bool FIRST_WhereDecls(int tt) {
	return tt == Tok_WHERE;
}
...
void Parser::inline_() {
	if( la.d_type == Tok_INLINE ) {
		expect(Tok_INLINE, false, "inline_");
	} else if( la.d_type == Tok_IN ) {
		expect(Tok_IN, false, "inline_");
	} else
		invalid("inline_");
}

void Parser::VariantPart() {
	expect(Tok_CASE, false, "VariantPart");
...
void Parser::FieldList() {
	if( FIRST_IdentList(la.d_type) ) {
		...
		if( la.d_type == Tok_BITS ) {
			...
			expect(Tok_integer, false, "FieldList");
		}
...
void Parser::GotoStatement() {
	expect(Tok_GOTO, false, "GotoStatement");
...
void Parser::RepeatStatement() {
	expect(Tok_REPEAT, false, "RepeatStatement");
	StatementSequence();
	expect(Tok_UNTIL, false, "RepeatStatement");
	expression();
}
...
void Parser::ProcedureType() {
	procedure();
	if( ( peek(1).d_type == Tok_Lpar && ( peek(2).d_type == Tok_Hat || peek(2).d_type == Tok_POINTER ) )  ) {
...
void Parser::ProcedureHeading() {
	procedure();
	if( FIRST_Receiver(la.d_type) || ( peek(1).d_type == Tok_ident && peek(2).d_type == Tok_Dot )  ) {
		if( FIRST_Receiver(la.d_type) ) {
			Receiver();
		} else if( ( peek(1).d_type == Tok_ident && peek(2).d_type == Tok_Dot )  ) {
			expect(Tok_ident, false, "ProcedureHeading");
			expect(Tok_Dot, false, "ProcedureHeading");
		} else
...
void Parser::ForwardDecl() {
	procedure();
	expect(Tok_Hat, false, "ForwardDecl");
	if( FIRST_Receiver(la.d_type) || ( peek(1).d_type == Tok_ident && peek(2).d_type == Tok_Dot )  ) {
		if( FIRST_Receiver(la.d_type) ) {
			Receiver();
		} else if( ( peek(1).d_type == Tok_ident && peek(2).d_type == Tok_Dot )  ) {
			expect(Tok_ident, false, "ForwardDecl");
			expect(Tok_Dot, false, "ForwardDecl");
		} else
			invalid("ForwardDecl");
	}
	identdef();
...
void Parser::module() {
	...
	if( FIRST_ModuleAttributes(la.d_type) ) {
		ModuleAttributes();
	}
	if( la.d_type == Tok_Semi ) {
		expect(Tok_Semi, false, "module");
	}
	while( FIRST_ImportList(la.d_type) || FIRST_WhereDecls(la.d_type) || FIRST_DeclarationSequence(la.d_type) ) {
		if( FIRST_ImportList(la.d_type) ) {
			ImportList();
		} else if( FIRST_WhereDecls(la.d_type) ) {
			WhereDecls();
		} else if( FIRST_DeclarationSequence(la.d_type) || la.d_type == Tok_BEGIN || la.d_type == Tok_CONST || la.d_type == Tok_END || la.d_type == Tok_TYPE || la.d_type == Tok_VAR || la.d_type == Tok_WHERE || la.d_type == Tok_PROCEDURE || la.d_type == Tok_IMPORT || la.d_type == Tok_PROC ) {
			DeclarationSequence();
		} else
			invalid("module");
	}
...
void Parser::ModuleAttributes() {
	expect(Tok_Lbrack, false, "ModuleAttributes");
	Attribute();
	while( la.d_type == Tok_Comma || FIRST_Attribute(la.d_type) ) {
		if( la.d_type == Tok_Comma ) {
