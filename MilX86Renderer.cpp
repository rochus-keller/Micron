...
// NOTE: the goal of this generator is to have something to work with;
//   optimization potential: use registers for intermediate results (base on MRL),
...
Renderer::Renderer(AstModel* mdl)
    : d_mdl(mdl), d_code(mdl, 4, 4), d_emitDwarf(false), d_cdeclReturns(false),
      ...
      d_elf(ElfWriter::ArchX86), d_currentModule(0)
{
...
quint32 Renderer::getOrEmitVtable(int vtableIdx)
{
    ...
    // are NULL when the base type is from an imported module (vtables for
    // imported types are created after the current module's types).
    QList<int> ancestors;
    {
    ...
    for (int k = MaxExtensions - 1; k >= 0; k--) {
        ...
        if (k <= extlev) {
            ...
            d_elf.addRelocation(d_sections.relData, entryPos, vtSym, R_386_32);
            d_elf.patchWord(d_sections.data, entryPos, 0);
        }
    }
...
quint32 Renderer::getOrCreateVtableSymbol(int vtableIdx)
{
    ...
    if (declaringModule == d_currentModule) {
        // Local type: define a global symbol at the vtable's data offset
        ...
        // Imported type: create an undefined (extern) symbol reference
...
bool Renderer::renderModule(Declaration* module)
{
    ...
    // Other modules emit UNDEF references to these symbols for isinst/ancestor arrays.
...
bool Renderer::generateMainObject(const QByteArrayList& moduleNames, const QString& outputPath, bool indirect)
{
    ...
    // Without this, debuggers show spurious "??" entries above the top frame.
    {
        ...
        const quint8 ESP_REG = 4, EBP_REG = 5, EIP_REG = 8;
        ...
        appendULEB128f(frame, 1);          // code_alignment_factor
        appendSLEB128f(frame, -4);         // data_alignment_factor
        appendULEB128f(frame, EIP_REG);    // return_address_register
        // Initial instructions: CFA = ESP + 4, EIP at CFA-4
        frame.append(char(DW_CFA_def_cfa_));
        appendULEB128f(frame, ESP_REG);
        appendULEB128f(frame, 4);
        frame.append(char(DW_CFA_offset_ | EIP_REG));
        appendULEB128f(frame, 1);          // EIP at CFA - 1*4
        ...
        // initial_location (relocated to .text + 0)
        elf.addRelocation(relFrameSec, frame.size(), textSym, 1 /*R_386_32*/);
        appendLE32f(frame, 0);             // _start offset (0)
        if (indirect) {
            appendLE32f(frame, em.currentPosition()); // covers _start + __mic$main
        } else {
            appendLE32f(frame, em.currentPosition()); // covers entire _start
        }
        // After xor ebp,ebp (2 bytes): mark EIP as undefined — no caller
        frame.append(char(DW_CFA_advance_loc_ | 2));
        frame.append(char(DW_CFA_undefined_));
        appendULEB128f(frame, EIP_REG);
        while ((frame.size() - fdeStart) % 4 != 0)
            frame.append(char(DW_CFA_nop_));
        patchLE32f(frame, fdeStart, frame.size() - fdeStart - 4);

        if (indirect) {
            // FDE for __mic$main: standard push ebp; mov ebp, esp prologue
            ...
            frame.append(char(DW_CFA_offset_ | EBP_REG));
            appendULEB128f(frame, 2);          // EBP at CFA - 2*4
            // mov ebp, esp (2 bytes)
            frame.append(char(DW_CFA_advance_loc_ | 2));
            frame.append(char(DW_CFA_def_cfa_register_));
            appendULEB128f(frame, EBP_REG);
...
int Renderer::emitOp(Procedure& proc, int pc)
{
    ...
    case LL_isinst: {
        ...
        // Compare with target vtable's canonical global symbol (must use
        // 32-bit immediate form because the value is subject to relocation).
        ...
        em.emitByte(0); em.emitByte(0); em.emitByte(0); em.emitByte(0);
        d_elf.addRelocation(d_sections.relText, immOff, vtSym, R_386_32);
